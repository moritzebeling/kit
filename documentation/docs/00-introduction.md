...
Prerendering means computing the contents of a page at build time and saving the HTML for display. This approach means that the page does not have to be recomputed for each visitor and so it scales well with the number of visitors. The tradeoff is that the build process is more expensive and prerendered content can only be updated by building and deploying a new version of the application.
